...
async function writeComment(){
    ...
    await store.dispatch("boardStore/getBoard", store.state.boardStore.board.articleNo); // 글 다시 불러오기
    comment.content = "";
}
...
async function deleteB(){
    await store.dispatch("boardStore/deleteB", store.state.boardStore.board.articleNo); // 글 삭제
		...
		<div class="container" data-aos="fade-up">
			<div class="row no-gutters" style="background-color: white">
				<div class="px-5 mb-3">
                    <div class="row">
						...
						<button type="button" id="btn-list"
							class="btn btn-outline-dark btn-sm me-1" v-on:click="list">글목록</button>
						<div v-if="store.state.userStore.userInfo.id == article.userId">
							...
							<button type="button" id="btn-delete"
								class="btn btn-outline-danger btn-sm ms-1" v-on:click="deleteB">삭제</button>
						</div>
					</div>
                    </div>
                        ...
                        <div>
                            <div class="mb-1">
                                <div class = "d-inline bg-danger opacity-8 px-1 py-2" v-if="article.isnotice">
                                    <h5 class = "d-inline text-white">
                                    공지
                                </h5>
                                </div>
                                &nbsp;
								<h3 class = "d-inline" v-text="article.subject">
                                </h3>
							</div>
							<div class="">
                                ...
                                <div class = "d-inline">
                                    ...
                                    &nbsp;
                                    <h6 class = "d-inline text-secondary font-weight-light" v-if="article.isnotice">관리자</h6>
                                </div> 
                                <div>
                                    <label for="id" class="form-labe font-weight-light">{{ article.date }}</label>
                                    &nbsp;
                                    <label for="id" class="form-labe font-weight-light">조회 {{ article.hit }}</label>
                                </div>
                            </div>
							...
							<div class="mb-4">
								...
								<textarea class="form-control p-2" rows="10" name="content"
									readonly="readonly">{{article.content}}</textarea>
							</div>
                            <div class ="d-inline">
                                <div class="d-inline" @click="recommend" @mouseover="changeCursor" @mouseout="restoreCursor">
                                    <img src="@/assets/img/heart6.png" class = "w-2 mb-2">
                                    <h6 class="d-inline px-1 font-weight-light">좋아요&nbsp;{{ article.recommendation }}</h6>
                                </div>
                                &nbsp;
                                <div class="d-inline">
                                    <img src="@/assets/img/comment1.png" class = "w-2 mb-2">
                                    <h6 class="d-inline px-1 font-weight-light">댓글&nbsp;{{ article.comment }}</h6>
                                </div>
                            </div>
						</div>
                ...
                <div class = "px-5">
                    ...
                    <comment-item v-for="(comment, index) in comments" :key="comment.commentNo" :comment = "comment">
                    </comment-item>
                </div>
                <div class = "mt-1 px-5">
                    <div class = "border border-dark">
            <h5 class = "mt-1 px-2 font-weight-bold" v-text="store.state.userStore.userInfo.name"></h5>
            <div>
                <textarea class="form-control mt-1 px-2" rows="2" placeholder="댓글을 남겨보세요" :value = "comment.content" @input="updateComment"></textarea>
            </div>
